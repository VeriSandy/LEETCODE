...
Task Scheduler --
    ...
    Finish task first which has higher occurrence.
*/
...
int leastInterval(char* tasks, int tasksSize, int n)
{
    ...
    int i = 0, j = 0, count = 0, totaCount = 0, flag = 0;
    ...
    */
    ...
    while(hash[0])
    {
        ...
        while(i <= n)
        {
            if(hash[0] == 0)
                break;
            //n > 25, there can't be any tasks for n bw 26 to 100
            if(i < 26 && hash[i] > 0)
                hash[i]--;
            count++;
            i++;
    ...
    /*
    This is worng, if you are finishing task sequentially irrespective of n, try the below test case
